export const SEED_APPS = [
  ...
  // === NEXTCLOUD ===
  {
    ...
    oidc: 1, oauth2: 0, saml2: 1, scim: 0, ldap: 1, kerberos: 0, ws_federation: 0, cas: 0,
    entra_id: 1, okta: 1, ping: 1, keycloak: 1, auth0: 1, onelogin: 1, forgerock: 0,
    license_requirement: 'standard',
    implementation_notes: 'SAML 2.0 via the free "SSO & SAML Authentication" app (Nextcloud App Store). OIDC via the "OpenID Connect Login" app. LDAP/AD built-in via User Auth LDAP app. No SCIM; users provisioned via LDAP or JIT from SAML/OIDC. Note: if server-side encryption is enabled, only LDAP is supported.',
    source_urls: JSON.stringify([
      { title: 'Nextcloud SAML SSO App', url: 'https://apps.nextcloud.com/apps/user_saml' },
      { title: 'Nextcloud OIDC Configuration', url: 'https://docs.nextcloud.com/server/stable/admin_manual/configuration_user/user_auth_oidc.html' },
      { title: 'Nextcloud LDAP Integration', url: 'https://docs.nextcloud.com/server/stable/admin_manual/configuration_user/user_auth_ldap.html' },
    ]),
    confidence: 90,
  },
  ...
  // === RANCHER ===
  {
    ...
    entra_id: 1, okta: 1, ping: 1, keycloak: 1, auth0: 1, onelogin: 1, forgerock: 0,
    license_requirement: 'standard',
    implementation_notes: 'Supports multiple auth backends natively in the open-source edition: SAML 2.0 (Okta, PingIdentity, ADFS, Keycloak, Shibboleth), OIDC (generic, Keycloak, Auth0, Dex), and LDAP/Active Directory. No SCIM; users provisioned on first login (JIT). Group membership from IdP drives RBAC. Configure under Global Settings > Authentication.',
    source_urls: JSON.stringify([
      { title: 'Rancher OIDC Configuration', url: 'https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/authentication-config/configure-generic-oidc' },
      { title: 'Rancher Okta SAML', url: 'https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/authentication-config/configure-okta-saml' },
      { title: 'Rancher Active Directory / LDAP', url: 'https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/authentication-config/configure-active-directory' },
    ]),
    confidence: 90,
  },
  ...
  // === INUVIKA ===
  {
    ...
    entra_id: 1, okta: 1, ping: 1, keycloak: 1, auth0: 0, onelogin: 0, forgerock: 0,
    license_requirement: 'enterprise',
    implementation_notes: 'SAML 2.0 for web portal and native client authentication (confirmed: Okta, Entra ID, Duo, Watchguard). LDAP/Active Directory for user directory sync. Kerberos for transparent SSO within Windows AD environments. No OIDC or SCIM support documented. Commercial product — no free community edition.',
    source_urls: JSON.stringify([
      { title: 'Inuvika SAML 2.0 Configuration Guide', url: 'https://docs.inuvika.com/3.4/saml_2.0_configuration_guide/' },
      { title: 'Inuvika Active Directory Kerberos SSO', url: 'https://docs.inuvika.com/3.3/active_directory_sso_using_kerberos/' },
      { title: 'Inuvika LDAP Integration Guide', url: 'https://docs.inuvika.com/3.5/ldap_integration_guide/' },
    ]),
    confidence: 85,
  },
  ...
  // === CHECKMK ===
  {
    app_key: 'checkmk',
    app_name: 'Checkmk',
    vendor: 'tribe29 GmbH / Checkmk GmbH',
    description: 'IT infrastructure and application monitoring platform.',
    oidc: 0, oauth2: 0, saml2: 1, scim: 0, ldap: 1, kerberos: 1, ws_federation: 0, cas: 0,
    entra_id: 1, okta: 1, ping: 1, keycloak: 1, auth0: 0, onelogin: 0, forgerock: 0,
    license_requirement: 'enterprise',
    implementation_notes: 'SAML 2.0 natively supported in commercial editions (Pro/Ultimate) from Checkmk 2.2+. Community (free) edition requires manual Apache mod_auth_mellon setup — not officially supported. LDAP/AD sync available in all editions. Kerberos possible via Apache mod_auth_kerb (manual, not officially supported). No OIDC or SCIM.',
    source_urls: JSON.stringify([
      { title: 'Checkmk SAML SSO', url: 'https://docs.checkmk.com/latest/en/saml.html' },
      { title: 'Checkmk LDAP/AD Integration', url: 'https://docs.checkmk.com/latest/en/ldap.html' },
      { title: 'Checkmk Kerberos SSO', url: 'https://docs.checkmk.com/latest/en/kerberos.html' },
    ]),
    confidence: 88,
  },
